...
/* ── 10 preset category colors ── */
const PRESET_COLORS = [
  { hex: '#0a1f44', label: 'Navy'     },
...
function ColorPicker({ value, onChange }) {
  return (
    <div>
      ...
      {value && (
        <p className="text-xs text-gray-400 mt-1">Selected: <span className="font-mono">{value}</span></p>
      )}
    </div>
  );
}
    ...
    <div className="space-y-5">
      ...
      {showForm && (
        <div className="card p-5 border-2 border-[#0a1f44]/20 space-y-4">
          ...
          {/* Color Picker — replaces free-text color input */}
...
/* ── Coming Soon placeholder for settings pages ── */
...
/* ── TAB META — used for breadcrumb labels ── */
const TAB_LABELS = {
  overview:   'Overview',
...
export default function AdminDashboard() {
  ...
  const Breadcrumb = () => {
    ...
    const settingsLabel = SETTINGS_LABELS[settingsPage] || settingsPage;
        ...
        <div className="flex items-center gap-1 text-sm text-gray-400 flex-wrap">
          ...
          {tab !== 'overview' && (
            <>
              ...
              <span className={isSettings ? 'text-gray-500 cursor-pointer hover:text-gray-700' : 'font-semibold text-[#0a1f44]'}
                onClick={isSettings ? () => {} : undefined}>
                {tabLabel}
              </span>
            </>
          )}
          {isSettings && (
            <>
              <ChevronRight className="w-3.5 h-3.5 flex-shrink-0" />
              <span className="font-semibold text-[#0a1f44]">{settingsLabel}</span>
            </>
          )}
        </div>
            ...
            <div className="card p-5">
              ...
              {stats?.categoryStats?.map(({ _id, count }) => (
                <div key={_id} className="flex items-center justify-between py-1.5 border-b border-gray-50 last:border-0 text-sm">
                  <span className="text-gray-700 truncate mr-2">{_id}</span><span className="font-bold text-[#0a1f44] flex-shrink-0">{count}</span>
                </div>
              ))}
            </div>
            <div className="card p-5">
      ...
      {tab === 'dealers' && (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative flex-1 sm:flex-none"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" /><input type="text" placeholder="Search dealers…" value={dealerSearch} onChange={e => setDealerSearch(e.target.value)} onKeyDown={e => e.key==='Enter' && fetchDealers(dealerSearch, dealerStateFilter)} className="input-field pl-9 text-sm w-52" /></div>
          ...
          <div className="sm:hidden space-y-3">
            ...
            {dealers.map(d => (
              <div key={d._id} className="card p-4 space-y-2">
                ...
                <p className="text-xs text-gray-400">Joined {new Date(d.createdAt).toLocaleDateString('en-IN')}</p>
      ...
      {tab === 'settings' && (
        <div className="flex flex-col md:flex-row gap-4 md:gap-6 min-h-[500px]">
          ...
          <aside className="w-full md:w-48 flex-shrink-0">
            <nav className="flex md:flex-col gap-1 overflow-x-auto no-scrollbar md:sticky md:top-24 pb-1 md:pb-0">
              {[
